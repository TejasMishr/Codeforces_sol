...
//Hello!! Stalker---->STOP STALKING :)
  //tot_of_digits
  ll tot_of_digits(ll n) {ll tot=0; while(n>0) {tot+=n%10; n/=10; } return tot;}
...
signed main(){
    ...
    while(t--){
        ...
        multiset<int> s;
        int tot=0;
        REP(i,0,n) tot +=vec[i] ,s.insert(tot);
        ...
        REP(i,0,n-1){
            ...
            s.erase(it);
            int nxt = *s.begin();
            if(i != 0) nxt -= tot , nxt += vec[i];
            mini = min(mini, nxt);
        }
        tot += vec[n - 1];
        print(tot - mini + abs(mini));
    }
}
