...
int main(){
  ...
  while(t--){
        ...
        REP(i,0,n) cin>>a[i];
        int m;
        vll prefix(n+2), suffix(n+2);
 
        REPE(i,1,n){
            prefix[i] = prefix[i-1];
            if(i>2 && (a[i-1] - a[i-2]) > (a[i-2] - a[i-3])) prefix[i] +=(a[i-1] - a[i-2]) - 1;
        }
 
        for (int i = n; i >= 0; i--){
            suffix[i] = suffix[i + 1];
            if (i<n-1 && (a[i] - a[i-1]) > (a[i+1] - a[i])) suffix[i] += (a[i] - a[i-1]) - 1;
        }
        cin>>m;
        while(m--){
            ll x, y;cin>>x>>y;
            cout << ((x<y) ? (y-x) + (prefix[y] - prefix[x]) : (x-y) + (suffix[y] - suffix[x]))<< endl;
        }
  }
  return 0;
}