...
void sieve(ll N) {
    ...
    vec[1] = 1,vec[0] = 0;
    for (int i = 2; i <= N; i++) {
        if (!vec[i]) vec[i] = i,pr.push_back(i);
        for (int j = 0; j < pr.size() && pr[j] <= vec[i] && i * pr[j] <= N; j++) vec[i * pr[j]] = pr[j];
    }
}
...
signed main() {
    ...
    set<int> st;
    sieve(N);
    REP(i, 0, pr.size() - 1) st.insert(pr[i] * pr[i]), pr[i] = pr[i] * pr[i];

    while (t--) {
        int n; cin >> n;
        bool b = false;
        if (n < 38) { print("No"); continue;}
        REP(i, 0, pr.size() - 1) {
            if (pr[i] * pr[i] >= n) { break; }
            for (int j = i + 1; j < pr.size() - 1; j++) {
                ll x = n - (pr[i] + pr[j]);
                if (x <= 0) break;
                if (st.find(x) != st.end() && x != pr[i] && x != pr[j]) { b = true; print("Yes"); break; }
            }
            if(b) break;
        }
        if(!b) print("NO");
