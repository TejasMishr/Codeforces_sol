...
int main() {
    ...
    while (t--) {
        ll n, m;cin >> n >> m;
         ...
         vll bat(n), ball(m);
         REP(i,0,n) cin >> bat[i];
         REP(i,0,m) cin >> ball[i];

        if (n < 4 || m < 4 || n+m < 11) {print(-1);}
        else {
            sort(bat.rbegin(), bat.rend());
            sort(ball.rbegin(), ball.rend());

            ll sum = accumulate(bat.begin(), bat.begin() + 4, 0LL) + accumulate(ball.begin(), ball.begin() + 4, 0LL);

            vll com(m + n - 8);
            merge(bat.begin() + 4, bat.end(), ball.begin() + 4, ball.end(), com.begin());
            sort(com.rbegin(), com.rend());

            sum += accumulate(com.begin(), com.begin() + 3, 0LL);
            print(sum);
        }
    }
