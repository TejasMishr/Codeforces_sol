...
//   }
    ...
    REP(i,1,n){
        if (vec[i] > vec[i-1]) dp1[i] = max(dp1[i], 1 + dp1[i-1]);
    }
   
    for (int j = n-2; j >= 0; j--){
        if (vec[j] < vec[j+1]) dp2[j] = max(dp2[j], 1 + dp2[j+1]);
    }
   
    ll ans = 1;


    REP(i,0,n) ans = max(ans, dp1[i]);
    // 
    ...
    REP(i, 1, n-1){
        if (vec[i-1] < vec[i+1] && (vec[i] <= vec[i -1] || vec[i] >= vec[i + 1])){
