...
vll dijkstra(ll V, vpll A[], ll S){
  ...
  while(!pq.empty()){
    ll dis=pq.top().first;
    ll low=pq.top().second;
    pq.pop();
    for(auto child:A[low]){
      ll next=child.f;
      ll nextDist=child.s;
      if(dist[next]>dist[low]+nextDist){
        dist[next]=dist[low]+nextDist;
...
ll nCrModPFermat(ll n,ll r, ll p){
    ...
    // Base case
    if (r == 0) return 1;
    // Fill factorial vecay so that we
...
int main(){
  ...
  while(t--){
        ...
        int i = 0;
        for (; i < a; i++) {
            if (vec[i] - low > 1) break;
            low = vec[i];
        }

        if (i == a) {
            if (mp.size() % 2 == 0) {print("Bob");}
            else {print("Alice");}
        } 
        else if (a == 1 || i % 2 == 0 || mp.size() == 1) {print("Alice")}
        else {print("Bob");}
  }
  return 0;
}