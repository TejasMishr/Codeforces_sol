...
ll nCrModPFermat(ll n,ll slv, ll p){
    // If n<slv, then nCr should return 0
    if (n < slv) return 0;
    // Base case
    if (slv == 0) return 1;
    // Fill factorial array so that we
    // can find all factorial of slv, n
    // and n-slv
    ll fac[1+n];
    fac[0] = 1;
    REPE(i,1,n)
        // fac[i] = mod_mul(fac[i-1],i);
        return (fac[n] * modInverse(fac[slv], p-2,p) % p * modInverse(fac[n - slv], p-2,p) % p) % p;
}
*/
//---------------------------------------------------------------------------------------------------------------------
ll dp[5010][5010], cnt[5010];
#define vec dp[i][ord]
ll slv(ll i, ll ord){
    if (ord < 0) return -2;
    if (!vec)  return vec;
    ...
    ((cnt[i]) ? vec = slv(i-1, ord-1) : vec = slv(i-1, ord));
    if (slv(i-1, ord) != -2 && slv(i-1, ord) + cnt[i] <= ord){

        if (vec == -2 || b){
            if(vec == -2 || vec > slv(i - 1, ord) + cnt[i]) vec = slv(i - 1, ord) + cnt[i];
        }
    }
    return vec;
}
int main(){
    ...
    while (t--){
        ll n; cin >> n;
        ll res = n;
        REPE(i, 0, n) fill_n(dp[i],1+n, -1);
        REP(i, 0, n) { 
            ll a; cin>>a;
            cnt[a]++; 
        }
        
        for(ll i=n; i>=0;--i) if(slv(n,i) != -2) res=i;
