...
int main(){
  ...
  while(t--){
        ...
        REP(i,0,x) cin >> vec[i];
        REP(i,0,x) cin >> vec2[i];

        unordered_map<ll, ll> ump,remc;

        REP(i,0,x) ump[vec[i] % y]++; 
        REP(i,0,x) remc[vec2[i] % y]++;

        

        for (const auto& cp : ump) {

            ll rem = cp.F, remneed = (y - rem) % y;
            if (remc.find(remneed) != remc.end()) cnt += cp.S * remc[remneed];
