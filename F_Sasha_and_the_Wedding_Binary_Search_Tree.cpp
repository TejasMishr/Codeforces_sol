...
long long dfs(int idx) {
    ...
    long long left_count = dfs(tree[idx].left);
    long long right_count = dfs(tree[idx].right);

    int left_value = (tree[idx].left == -1) ? 1 : tree[tree[idx].left].value + 1;
    int right_value = (tree[idx].right == -1) ? tree[idx].value : tree[tree[idx].right].value - 1;

    int num_values = right_value - left_value + 1;

    return dp[idx] = (left_count * right_count) % MOD * dfs(num_values - 1) % MOD;
}
