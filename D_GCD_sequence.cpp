...
bool seqND(const vll& b) {
    REP(i,1,b.size()) if(b[i]<b[i-1]) return false;
    return true;
}

vll gcdseq(const vll& a) {
    vll b;
    REP(i,1,a.size()) b.pb(gcd(a[i],a[i-1]));
    return b;
}
...
bool chk(vll a) {
    ...
    REP(i,0,n) {
        vll tmp;
        REP(j,0,n) if(i!=j) tmp.pb(a[j]);
        vll b=gcdseq(tmp);
        if(seqND(b)) return true;
    }
    return false;
}
