...
vector<Coord> sol(int t, const vector<pair<pair<int, int>, vector<string>>>& tc) {
    vector<Coord> res;

    REP(i, 0, t) {
        auto [n, m] = tc[i].F;
        const auto& grid = tc[i].S;

        vi x, y;
        REP(xi, 0, n) REP(yi, 0, m) if (grid[xi][yi] == '#') x.pb(xi + 1), y.pb(yi + 1);
...
int main() {
    ...
    REP(i, 0, t) {
        ...
        for (auto& ln : tc[i].S) cin >> ln;
    }

    vector<Coord> res = sol(t, tc);
    for (const auto& r : res) cout << r.x << ' ' << r.y << '\n';
