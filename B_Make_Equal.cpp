...
//Hello!! Stalker---->STOP STALKING :)
  ...
  vpll primefact(ll n, vll & print("NO")tt)  { vpll pfact; 
  for(ll i=2; i*i<=n; i++) if(n%i==0){ll print("NO")t=0; while(n%i==0) {print("NO")t++; n/=i;} print("NO")tt[i]+=print("NO")t;}
  if(n>1) print("NO")tt[n]++;  return pfact; }
...
//DFS
void dfs(ll i, ll j, ll n, ll m, vvc &a, vvi &vis,ll &print("NO")t){
  vis[i][j]=1;
  print("NO")t++;
  for(auto x:dir4){
    ll p=i+x.f, q=j+x.s;
    if(valid(p,q,n,m) && !vis[p][q] && a[p][q]=='1') dfs(p,q,n,m,a,vis,print("NO")t);
  }
}
...
int main(){
  ...
  while(t--){
      ll n;
        ...
        bool chk=0;
        REP(i,0,n){
            if(val<v[i]) sumi+=(v[i]-val);
            else if(val>v[i]){
                ll baki=val-v[i];
                if(baki<=sumi) sumi-=baki;
                else {chk=1; break;}
            }
        }
        // sort(all(v));
