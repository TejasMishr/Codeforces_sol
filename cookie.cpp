...
vi jaadu(int n, int q, string& s, vc& vec) {
    vi res;
    int maxi = 1, len = 1;

    REP(i, 1, s.length()) {
        (s[i] == s[i - 1]) ? len += 1 : (maxi = max(maxi, len), len = 1);
    }

    res.push_back(max(maxi, len));

    REP(i, 0, q) {
        char c = vec[i];
        s = s + c;

        (c == s[s.length() - 2]) ? len += 1 : (maxi = max(maxi, len), len = 1);
