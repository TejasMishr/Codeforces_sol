...
bool chk(vi& a, vi& b, int h, int w) {
    ...
    for (int i = h - 1; i >= 1; --i) {
        for (int j = w - 1; j >= 1; --j) {
            ...
            if (a[i * w + j] != b[i * w + j]) {
                ...
                int d = (b[i * w + j] - a[i * w + j] + 3) % 3;

                a[i * w + j] = b[i * w + j];
                // print(i * w + j);
                a[(i - 1) * w + j] = (a[(i - 1) * w + j] + 2 * d) % 3;
                a[i * w + j - 1] = (a[i * w + j - 1] + 2 * d) % 3;
                // print(a[(i - 1) * w + j - 1]);
                a[(i - 1) * w + j - 1] = (a[(i - 1) * w + j - 1] + d) % 3;
            }
        }
    }
    REP(i,0,h) if (a[i * w] != b[i * w]) return false;
    REP(j,0,w) if (a[j] != b[j]) return false;
    return true;
}
...
int main() {
    ...
    while (t--) {
        ...
        print((chk(gridA, gridB, r, c) ? "YES" : "NO"));
    }
    return 0;
}
