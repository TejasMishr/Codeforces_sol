...
void cal(int& n, int& l, int& r, vll& vec, vll& pre, set<ll>& st, int& cnt){
    REPE(j,1,n) {
            ll cur= pre[j];
            auto lower=st.lower_bound(cur- r), upper=st.upper_bound(cur- l);

            if (lower !=st.end() && lower != upper) {
                cnt+= 1;
                st.clear(), st.insert(cur);  
            } 
            else  st.insert(cur);
        }
}
...
int main(){
  ...
  while(t--){
        ...
        REP(i,0,n) cin>>vec[i];
        REPE(i,1,n) pre[i] = pre[i - 1] + vec[i - 1];

        set<ll> st;
        st.insert(0); 
        cal(n,l,r,vec,pre,st,cnt);
