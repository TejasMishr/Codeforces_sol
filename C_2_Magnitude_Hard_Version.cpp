...
int main() {
    ...
    while (x--) {
        ...
        for (int i = 2; i <= n; i++) {
            dp1[i] = max({dp1[i - 1] + a[i], abs(dp1[i - 1] + a[i]), dp2[i - 1] + a[i], abs(dp2[i - 1] + a[i])});

            x1[i] = (dp1[i] == dp1[i - 1] + a[i]) ? (x1[i] + x1[i - 1]) % MOD : x1[i];
            x1[i] = (dp1[i] == abs(dp1[i - 1] + a[i])) ? (x1[i] + x1[i - 1]) % MOD : x1[i];

            if (dp1[i - 1] != dp2[i - 1]) {
                x1[i] = (dp1[i] == dp2[i - 1] + a[i]) ? (x1[i] + x2[i - 1]) % MOD : x1[i];
                x1[i] = (dp1[i] == abs(dp2[i - 1] + a[i])) ? (x1[i] + x2[i - 1]) % MOD : x1[i];
            }

            dp2[i] = min({dp1[i - 1] + a[i], abs(dp1[i - 1] + a[i]), dp2[i - 1] + a[i], abs(dp2[i - 1] + a[i])});

            x2[i] = (dp2[i] == dp1[i - 1] + a[i]) ? (x2[i] + x1[i - 1]) % MOD : x2[i];
            x2[i] = (dp2[i] == abs(dp1[i - 1] + a[i])) ? (x2[i] + x1[i - 1]) % MOD : x2[i];

            if (dp1[i - 1] != dp2[i - 1]) {
                x2[i] = (dp2[i] == dp2[i - 1] + a[i]) ? (x2[i] + x2[i - 1]) % MOD : x2[i];
                x2[i] = (dp2[i] == abs(dp2[i - 1] + a[i])) ? (x2[i] + x2[i - 1]) % MOD : x2[i];
            }
        }
